...
void main() {
  group('normalizarFoto', () {
    test(
      'una imagen más ancha que 1000px se redimensiona a 1000 de ancho',
      () async {
    ...
    test(
      'recomprime a JPEG calidad 80 — una foto subida a máxima calidad sale más liviana '
      '(el peso bajo es el motivo por el que existe esta función: mantener rescates '
      'y sus fotos livianos para el feed)',
      () async {
        final archivo = await _crearImagenTemp(
          'pesada.jpg',
          width: 1200,
    ...
    test('un timeout corto hace que se rinda pronto en vez de colgarse para '
      ...
      try {
        ...
        // sigan su camino.
        expect(
          DateTime.now().difference(arranque),
    ...
    test('normaliza varias fotos en paralelo sin que se pisen entre sí '
      ...
      final archivo1 = await _crearImagenTemp(
        'par1.jpg',
        width: 2000,
        height: 1000,
      );
      final archivo2 = await _crearImagenTemp(
        'par2.jpg',
        width: 1500,
        height: 2000,
      );
      try {
        ...
        expect(
          img.decodeImage(resultados[1])!.height,
          greaterThan(1000),
          reason:
              'la segunda imagen es más alta que ancha (1500x2000) — '
