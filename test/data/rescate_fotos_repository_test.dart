...
void main() {
  ...
  group('RescateFotosRepository', () {
    ...
    test('subir() CANCELA la tarea real al vencer el timeout, no solo deja '
      ...
      expect(
        task.canceladaDeVerdad,
        true,
        reason:
            'sin esto, la "subida" real seguía corriendo de fondo '
    ...
    test('eliminar() borra un archivo que existe', () async {
      ...
      await expectLater(
        repo.eliminar(rescateId: 'rescate-2', slot: 1),
        completes,
      );
    });

    test(
      'eliminar() ignora el error object-not-found (borrar un slot que nunca tuvo foto)',
      () async {
        final storage = MockFirebaseStorage();
        final ref = MockReference();
        when(() => storage.ref(any())).thenReturn(ref);
    ...
    test(
      'eliminar() propaga cualquier otro error que no sea object-not-found',
      () async {
        final storage = MockFirebaseStorage();
        final ref = MockReference();
        when(() => storage.ref(any())).thenReturn(ref);
    ...
    test('moverFoto() mueve el ARCHIVO de verdad (no solo la URL): los bytes '
      ...
      final url = await repo.moverFoto(
        rescateId: 'rescate-m',
        deSlot: 2,
        aSlot: 1,
      );
      ...
      expect(
        await storage.ref('rescates/rescate-m/foto1.jpg').getData(),
        bytes,
      );
      expect(
        await storage.ref('rescates/rescate-m/foto2.jpg').getData(),
        isNull,
        reason:
            'el archivo de origen tiene que borrarse — si queda, una '
            'segunda foto agregada después lo pisa y cambia la foto 1 en silencio',
      );
    });

    test(
      'moverFoto() devuelve null sin tocar nada si el origen no existe '
      '(ficha ya dañada por el bug: fotoUrl apuntando a un archivo borrado)',
      () async {
        final storage = fsm.MockFirebaseStorage();
        final repo = RescateFotosRepository(storage: storage);
    ...
    test(
      'moverFoto() también devuelve null cuando el SDK real tira '
      'object-not-found en la descarga (el mock de arriba devuelve null; '
      'Firebase de verdad lanza excepción — las dos ramas existen)',
      () async {
        final storage = MockFirebaseStorage();
        final ref = MockReference();
        when(() => storage.ref(any())).thenReturn(ref);
        ...
        final repo = RescateFotosRepository(storage: storage);
        expect(
          await repo.moverFoto(rescateId: 'r1', deSlot: 2, aSlot: 1),
          isNull,
        );
    ...
    test('moverFoto() no se queda esperando para siempre si la descarga '
      ...
      when(
        () => ref.getData(any()),
      ).thenAnswer((_) => Completer<Uint8List?>().future); // nunca se resuelve
    ...
    group(
      'urlApuntaASlot() — única regla de "¿este campo apunta a este slot?"',
      () {
        ...
        test(
          'reconoce el slot en una URL real con el path codificado (%2F)',
          () {
            expect(RescateFotosRepository.urlApuntaASlot(urlReal, 2), true);
            expect(
              RescateFotosRepository.urlApuntaASlot(urlReal, 1),
              false,
        ...
        test('null y URLs malformadas devuelven false sin lanzar — una '
          ...
          expect(
            RescateFotosRepository.urlApuntaASlot('%zz-malformada', 2),
            false,
          );
    ...
    test(
      'eliminarTodas() borra los dos slots posibles, ignorando los que no existían',
      () async {
        final storage = fsm.MockFirebaseStorage();
        final repo = RescateFotosRepository(storage: storage);
        await repo.subir(
    ...
    test('eliminarTodas() devuelve un Future<void> real: un .catchError((_) {}) '
      ...
      await expectLater(repo.eliminarTodas('r1').catchError((_) {}), completes);
    });
  });
}
