...
import 'package:salva_patitas/domain/compatibilidad.dart';

void main() {
  group('calcularCompatibilidad', () {
    test(
      'mapa vacío usa los defaults optimistas de cada campo (score perfecto)',
      () {
    ...
    group('energía', () {
      test('Activo con 8 horas o menos suma 20', () {
        final score = calcularCompatibilidad({
          'animalEnergia': 'Activo',
          'horasFuera': 8,
        });
      ...
      test(
        'Activo con más de 8 horas solo suma 10 (penaliza, no descarta)',
        () {
          final conPoco = calcularCompatibilidad({
            'animalEnergia': 'Activo',
            'horasFuera': 8,
          });
          final conMucho = calcularCompatibilidad({
            'animalEnergia': 'Activo',
            'horasFuera': 9,
          });
      ...
      test(
        'Muy activo con solo UNO de los dos factores (patio u horas) suma la mitad (10)',
        () {
    ...
    group('tamaño', () {
      test('Mediano en apartamento sin área exterior suma solo 10, no 20', () {
        final conEspacio = calcularCompatibilidad({
          'animalTamano': 'Mediano',
          'vivienda': 'Apartamento con balcón',
        });
    ...
    group('niños, mascotas y experiencia (mismo patrón en los tres)', () {
      test(
        'si el adoptante no tiene niños/mascotas, el requisito del animal no importa',
        () {
      ...
      test('si hay niños y el animal no es apto, penaliza 20 puntos', () {
        final apto = calcularCompatibilidad({
          'animalOkConNinos': true,
          'tieneNinos': true,
        });
        final noApto = calcularCompatibilidad({
          'animalOkConNinos': false,
          'tieneNinos': true,
        });
      ...
      test(
        'si el animal requiere experiencia y el adoptante no tiene, penaliza 20 puntos',
        () {
    ...
    test(
      'horasFuera llega como String desde Firestore (dato legado) y se interpreta igual que int',
      () {
        final comoInt = calcularCompatibilidad({
          'animalEnergia': 'Activo',
          'horasFuera': 9,
        });
        final comoString = calcularCompatibilidad({
          'animalEnergia': 'Activo',
          'horasFuera': '9',
        });
    ...
    test('horasFuera no numérico cae a 0 en vez de romper', () {
      ...
      final invalido = calcularCompatibilidad({
        'animalEnergia': 'Activo',
        'horasFuera': 'mucho',
      });
      final cero = calcularCompatibilidad({
        'animalEnergia': 'Activo',
        'horasFuera': 0,
      });
  ...
  group('explicarCompatibilidad', () {
    test(
      'siempre devuelve exactamente 5 razones (energía, tamaño, niños, mascotas, experiencia)',
      () {
        expect(explicarCompatibilidad({}).length, 5);
        expect(
          explicarCompatibilidad({
            'animalEnergia': 'Muy activo',
            'animalTamano': 'Grande',
            'tieneNinos': true,
            'tieneMascotas': true,
    ...
    test(
      'energía Activo con muchas horas solo da razón negativa con las horas reales interpoladas',
      () {
    ...
    test(
      'energía Muy activo sin patio y muchas horas da la razón más negativa de las cuatro variantes',
      () {
        final razones = explicarCompatibilidad({
          'animalEnergia': 'Muy activo',
          'vivienda': '',
    ...
    test(
      'niños: sin niños en casa es siempre positivo sin importar si el animal es apto',
      () {
        final razones = explicarCompatibilidad({
          'animalOkConNinos': false,
          'tieneNinos': false,
    ...
    test(
      'experiencia: el animal la requiere y el adoptante no tiene es negativo',
      () {
        final razones = explicarCompatibilidad({
          'animalRequiereExp': true,
          'experienciaPrevia': false,
    ...
    test(
      'experiencia: no se requiere es siempre positivo sin importar el adoptante',
      () {
        final razones = explicarCompatibilidad({
          'animalRequiereExp': false,
          'experienciaPrevia': false,
    ...
    test(
      'Finca cuenta como "tiene patio" en la explicación, igual que en el puntaje',
      () {
        final razones = explicarCompatibilidad({
          'animalEnergia': 'Muy activo',
          'animalTamano': 'Grande',
        ...
        expect(
          ...
          true,
          reason:
              'energía: Finca + pocas horas solo debería ser positivo, como Casa con jardín',
        );
        ...
        expect(
          ...
          true,
          reason:
              'tamaño: un animal grande en una Finca tiene espacio de sobra',
        );
  ...
  group(
    'el puntaje y la explicación nunca se contradicen (todas las combinaciones)',
    () {
      test(
        'cada criterio "positivo" en la explicación aporta más puntos que "negativo" del mismo criterio, para cada combinación de vivienda/energía/tamaño',
        () {
          const viviendas = [
            '',
            'Casa con jardín',
                ...
                for (final horas in [0, 5, 6, 8, 9, 12]) {
                  ...
                  if (positivos == 5) {
                    expect(
                      ...
                      reason:
                          '5/5 razones positivas pero score bajo ($score) para $sol',
                    );
                  }
                  if (negativos == 5) {
                    expect(
                      ...
                      reason:
                          '0/5 razones positivas pero score alto ($score) para $sol',
                    );
                  }
                }
              }
